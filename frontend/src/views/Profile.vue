...
// === Accordion 開閉控制 — section 預設展開狀態（gamification 開、其他收）===
...
<template>
  <div class="px-5 md:px-8 pt-10 pb-10 max-w-md md:max-w-4xl lg:max-w-5xl mx-auto space-y-5">
    <!-- ============ 頭部：頭像 / 暱稱 / 等級 / streak ============ -->
    <header class="text-center space-y-2">
      ...
      <h1 class="font-display text-2xl font-bold text-peach-500">
        {{ user?.display_name ?? user?.name ?? t('profile_greeting_default') }}
      </h1>
      ...
      <div
        ...
        class="mt-4 grid grid-cols-3 gap-2 max-w-sm mx-auto"
        data-test="profile-hero-stats"
      >
        <div class="bg-white/80 rounded-2xl py-2.5 shadow-soft">
          <p class="font-zen text-[10px] text-stone-400">{{ t('profile_level_label') }}</p>
          <p class="font-display font-bold text-peach-500 text-xl leading-none mt-1">{{ journey.level }}</p>
        </div>
        <div class="bg-white/80 rounded-2xl py-2.5 shadow-soft">
        ...
        <div class="bg-white/80 rounded-2xl py-2.5 shadow-soft">
          <p class="font-zen text-[10px] text-stone-400">XP</p>
          <p class="font-display font-bold text-peach-500 text-xl leading-none mt-1">{{ xp }}</p>
        </div>
      </div>
    </header>

    <div class="md:grid md:grid-cols-2 md:gap-5 md:items-start space-y-4 md:space-y-0">

    <!-- ============ Section 1: 遊戲化（寵物 / 等級 / 成就） ============ -->
    <section data-test="section-gamification" class="md:col-span-1">
      <button
        type="button"
        class="sticky top-0 z-10 w-full flex items-center justify-between px-5 py-3 rounded-3xl bg-white/95 backdrop-blur shadow-soft mb-3"
        :aria-expanded="openSections.gamification"
        @click="toggleSection('gamification')"
      >
        <span class="font-display font-bold text-peach-500 text-base flex items-center gap-2">
          <span>🐾</span>{{ t('profile_section_pet') }}
        </span>
        <span
          class="text-stone-400 transition-transform"
          :class="openSections.gamification ? 'rotate-180' : ''"
        >▾</span>
      </button>

      <div v-show="openSections.gamification" class="space-y-4">
        <!-- 我的寵物 -->
        ...
        <!-- 跳轉連結（journey / bbt / partner / week-report / pms / 婕樂纖深層） -->
    ...
    <!-- ============ Section 2: 訂閱 ============ -->
    <section v-if="subStatus" data-test="section-subscription" class="md:col-span-1">
      <button
        type="button"
        class="sticky top-0 z-10 w-full flex items-center justify-between px-5 py-3 rounded-3xl bg-white/95 backdrop-blur shadow-soft mb-3"
      ...
      >
        ...
        <span class="flex items-center gap-2">
          ...
          <span
            class="text-stone-400 transition-transform"
            :class="openSections.subscription ? 'rotate-180' : ''"
          >▾</span>
        </span>
      </button>

      <div v-show="openSections.subscription" class="space-y-3">
        <Card tone="plain" class="space-y-3" data-test="subscription-card">
          <RouterLink
            to="/me/premium"
            data-test="link-premium"
            class="flex items-center justify-between -mx-5 -mt-5 px-5 py-4 hover:bg-peach-50 transition-colors border-b border-cream-100"
            @click="sfx.play('ui_tap')"
          >
            <span class="font-zen text-peach-500 text-sm">
              {{ ent.isPremium() ? t('profile_link_premium_manage') : t('profile_link_premium_view') }}
            </span>
            <span class="text-stone-400">→</span>
          </RouterLink>
    ...
    <!-- ============ Section 4: 個人化（語言 / inclusive / 通知 / 音效）============ -->
    <section data-test="section-personalize" class="md:col-span-1">
      <button
        type="button"
        class="sticky top-0 z-10 w-full flex items-center justify-between px-5 py-3 rounded-3xl bg-white/95 backdrop-blur shadow-soft mb-3"
        :aria-expanded="openSections.personalize"
        @click="toggleSection('personalize')"
      >
        <span class="font-display font-bold text-peach-500 text-base flex items-center gap-2">
          <span>✨</span>{{ t('profile_section_personalize') }}
        </span>
        <span
          class="text-stone-400 transition-transform"
          :class="openSections.personalize ? 'rotate-180' : ''"
        >▾</span>
      </button>

      <div v-show="openSections.personalize" class="space-y-4">
        <!-- inclusive + locale -->
    ...
    <!-- ============ Section 5: 安全與隱私（鎖 / 匯出 / 刪除）============ -->
    <section data-test="section-security" class="md:col-span-1">
      <button
        type="button"
        class="sticky top-0 z-10 w-full flex items-center justify-between px-5 py-3 rounded-3xl bg-white/95 backdrop-blur shadow-soft mb-3"
      ...
      >
        ...
        <span
          class="text-stone-400 transition-transform"
          :class="openSections.security ? 'rotate-180' : ''"
        >▾</span>
      </button>

      <div v-show="openSections.security" class="space-y-4">
        <!-- 生物辨識鎖 -->
    ...
    <!-- ============ Section 6: 幫助 ============ -->
    <section data-test="section-help" class="md:col-span-1">
      <button
        type="button"
        class="sticky top-0 z-10 w-full flex items-center justify-between px-5 py-3 rounded-3xl bg-white/95 backdrop-blur shadow-soft mb-3"
      ...
      >
        ...
        <span
          class="text-stone-400 transition-transform"
          :class="openSections.help ? 'rotate-180' : ''"
        >▾</span>
      </button>
    ...
    <!-- ============ Section 7: 關於 ============ -->
    <section data-test="section-about" class="md:col-span-1">
      <button
        type="button"
        class="sticky top-0 z-10 w-full flex items-center justify-between px-5 py-3 rounded-3xl bg-white/95 backdrop-blur shadow-soft mb-3"
      ...
      >
        ...
        <span
          class="text-stone-400 transition-transform"
          :class="openSections.about ? 'rotate-180' : ''"
        >▾</span>
      </button>

      <div v-show="openSections.about" class="space-y-3">
        ...
        <div class="flex justify-center gap-4 text-[11px] text-stone-400 font-zen">
    ...
    <div class="pt-4 max-w-md mx-auto" style="padding-bottom: env(safe-area-inset-bottom)">
      <Button variant="secondary" full data-test="logout" sfx="ui_close" @click="doLogout">
        {{ t('profile_logout') }}
      </Button>
    </div>
  </div>
